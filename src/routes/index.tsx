  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: products, error: prodErr } = await supabase
        .from("products")
        .select(
          "id, name, description, image_url, sort_order, product_variants(id, name, price, stock, sort_order, is_active)",
        )
...
function Shop() {
  ...
  function addToCart(product: Product, variant: Variant) {
    setCart((prev) => {
      ...
      const qty = Math.min((current?.qty ?? 0) + 1, variant.stock);
      return {
        ...prev,
        [variant.id]: {
  ...
  function setQty(variantId: string, qty: number) {
    setCart((prev) => {
      ...
      if (!line) return prev;
      const next = Math.max(0, Math.min(qty, line.stock));
      if (next === 0) {
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {money(variant.price, currency)}
                            {!soldOut && variant.stock <= 10
                              ? ` · ${variant.stock} ${t("shop.left")}`
                              : ""}
                          </p>
                        </div>
                        {soldOut ? (
                          ...
                          <Button size="sm" className="h-9" onClick={() => addToCart(product, variant)}>
                            {t("shop.add")}
                          </Button>
                        )}
                      </li>
